...
import { Trophy, LogOut, Menu, X, ClipboardList } from 'lucide-react'
import { useAuth } from '../../hooks/useAuth'
import ThemeToggle from '../ui/ThemeToggle'

const NAV = [
  { to: '/mis-partidos', label: 'Mis Partidos', icon: ClipboardList, end: true },
]
    ...
    <div className="flex flex-col h-full">
      <div className="p-5 border-b" style={{ borderColor: 'var(--color-border)' }}>
        <div className="flex items-center gap-3">
          ...
          >
            <Trophy size={18} style={{ color: '#020617' }} />
          </div>
          <div className="min-w-0">
            <p className="font-semibold text-sm truncate" style={{ color: 'var(--color-fg)' }}>
              {user?.nombre || 'Árbitro'}
            </p>
            <p className="text-xs truncate" style={{ color: 'var(--color-fg-muted)' }}>Árbitro</p>
          </div>
        </div>
      </div>
